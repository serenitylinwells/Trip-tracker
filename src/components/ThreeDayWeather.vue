<template>
    <header>
        ...
        <nav>
            <div>
                <router-link to="/three-days" class="three-link">Three Days</router-link>
    ...
    <div id="weather" class="weather-forecast">
        <!-- 使用v-for指令循环天气数据 -->
        <div v-for="day in forecast" :key="day.fxDate" class="weather-day">
            <div class="weather-icon"><i :class="`qi-${day.iconDay}`"></i></div>
            <h3>Date: {{ day.fxDate }}</h3>
            <p>Max Temperature: {{ day.tempMax }}°C</p>
            <p>Min Temperature: {{ day.tempMin }}°C</p>
            <p>Daytime Weather: {{ day.textDay }}</p>
            <p>Nighttime Weather: {{ day.textNight }}</p>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        fetchWeather() {
            const location = "101280800"; // 佛山城市代码
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.code === "200") {
                        this.forecast = data.daily;// 请求成功，存储天气预报数据
                    } else {
